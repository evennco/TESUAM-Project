...
interface CardProps {
  title: string;
  description: string;
  onOpenModal: () => void; // callback para abrir el modal
}

const Card: React.FC<CardProps> = ({ title, description, onOpenModal }) => {
  return (
    <div className="flex flex-col items-center justify-between p-6 bg-white border border-gray-200 rounded-lg shadow-md">
